...
class StopFilter(BaseFilter):
    ...
    def __init__(self, mgr, name, data):
        ...
        # grun type
        self.grunt_contains = self.evaluate_attribute(  # f.gn matches e.gn
            event_attribute='gruntType', eval_func=operator.contains,
            limit=BaseFilter.parse_as_set(
                int, 'grunt_contains', data))
        
        # display
        self.min_dist = self.evaluate_attribute(  # f.min_dist <= m.distance
            event_attribute='pokestopdisplay', eval_func=operator.eq,
            limit=BaseFilter.parse_as_type(int, 'pokestopdisplay', data))
    ...
    def to_dict(self):
        ...
        if self.pokestopdisplay is not None:
            settings['pokestopdisplay'] = self.pokestopdisplay
